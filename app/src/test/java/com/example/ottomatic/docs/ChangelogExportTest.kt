...
class ChangelogExportTest {
    ...
    fun `the build agrees with the changelog`() {
        val name = System.getProperty("ottomatic.version.name").orEmpty()
        val code = System.getProperty("ottomatic.version.code").orEmpty()
        if (name.isEmpty() && code.isEmpty()) {
            println("[changelog] no ottomatic.version.* properties; run through Gradle to check.")
            return
        }
    ...
    private companion object {
        ...
        val REGENERATING = System.getProperty("ottomatic.changelog.regenerate") == "true"
