...
class ToolSpecTest {
    ...
    fun `pinned config is read off the trailing JSON`() {
        val spec = ToolSpec.parse("""action.notify {"title":"Easymatic"}""").single()
        assertEquals(ToolTarget.Node(NodeTypeId("action.notify")), spec.target)
        assertEquals(mapOf(ConfigKey("title") to "Easymatic"), spec.pinned)
    }
    ...
    fun `every shape round-trips through encode and parse`() {
        val specs = listOf(
            node("value.battery"),
            node("action.notify", "title" to "Easymatic", "message" to "hi"),
