...
import com.example.ottomatic.core.service.PromptFieldKind
...
class OverlayPrompts(context: Context) : Prompts {
    ...
    /**
     * The context every dialog is built from: Ottomatic's Material 3 theme, then
    ...
    private val themed: Context = DynamicColors.wrapContextIfAvailable(
        ContextThemeWrapper(context.applicationContext, R.style.Theme_Ottomatic_Dialog),
    )
    ...
    override suspend fun ask(request: PromptRequest): PromptAnswer {
        if (!Settings.canDrawOverlays(themed)) {
            return PromptAnswer.Unavailable("Ottomatic may not draw over other apps")
        }
    ...
    private fun fillValue(builder: MaterialAlertDialogBuilder, request: PromptRequest, answered: Answered) {
        ...
        builder.setIcon(R.drawable.ic_dialog_ottomatic)
