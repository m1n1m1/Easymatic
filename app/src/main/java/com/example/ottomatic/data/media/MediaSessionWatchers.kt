package com.example.ottomatic.data.media
...
import com.example.ottomatic.engine.trigger.MediaEventCodec
import com.example.ottomatic.engine.trigger.PlaybackDiff
...
class MediaSessionWatchers(context: Context) {
    ...
    private fun register(onReport: (String, LogLevel) -> Unit) {
        ...
        if (!registered) {
            ...
            onReport(
                "Ottomatic does not have notification access, so it cannot tell what any media player is doing",
                LogLevel.WARN,
            )
            return
        }
