...
internal object ScreenCapture {
    ...
    private val callbackExecutor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "ottomatic-screenshot").apply { isDaemon = true }
    }
    ...
    suspend fun grab(): ScreenGrab = gate.withLock {
        ...
        val service = OttomaticAccessibilityService.instance ?: return@withLock ScreenGrab.Failed(NOT_ENABLED)
        try {
            takeOne(service)
    ...
    private const val NOT_ENABLED =
        "Ottomatic needs accessibility access to take a screenshot — turn it on in " +
            "Settings, Accessibility, Ottomatic"
