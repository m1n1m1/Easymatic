 ...
 * ports after this node runs. Optionally logs a reason (EXPR-interpolated)
 * before halting. Pairs with `trigger.macro_finished`.
 *
...
class StopAction : Action {
    ...
    override suspend fun execute(input: ActionInput, context: ExecutionContext): ActionResult {
        val reason = input.config.expr("reason", default = "")
