...
import io.github.m1n1m1.easymatic.plugin.IEasymaticPlugin
...
const val PLUGIN_SERVICE_ACTION = "io.github.m1n1m1.easymatic.action.PLUGIN"

/**
 * The action a plugin app's **own settings Activity** advertises, if it has one.
 *
 * The second and last thing Easymatic reads out of a plugin's Android manifest, and the
 ...
 */
const val PLUGIN_SETTINGS_ACTION = "io.github.m1n1m1.easymatic.action.PLUGIN_SETTINGS"
 ...
 * priority the user granted Easymatic. A plugin gets an ordinary bound-service
...
class PluginConnections(
    ...
    suspend fun binder(packageName: String): IEasymaticPlugin? = mutex.withLock {
    ...
    private inner class Connection(private val packageName: String) : ServiceConnection {
        ...
        private var pending = CompletableDeferred<IEasymaticPlugin?>()
        private var binder: IEasymaticPlugin? = null

        suspend fun await(): IEasymaticPlugin? =
            binder ?: withTimeoutOrNull(BIND_TIMEOUT_MS) { pending.await() }

        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            val stub = service?.let { IEasymaticPlugin.Stub.asInterface(it) }
            binder = stub
