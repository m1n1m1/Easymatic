...
internal class SafFileStore(private val context: Context) : FileStore {
    ...
    private fun noGrant(path: FilePath): String = if (isDownloadRoot(path.parentPath())) {
        "Easymatic has not been given access to $path. Android does not allow granting the " +
        ...
        "Easymatic has not been given access to $path — add the folder under Folder access"
    }
