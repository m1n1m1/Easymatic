...
/**
 * The system pages where the grants Easymatic needs are turned on and off.
 *
...
internal fun Context.openSettingsFor(type: PrerequisiteType) {
    val intent = when (type) {
        ...
        // one toggle here that is not about Easymatic at all. It also toggles both
...
internal fun Context.openRevokeFor(type: PrerequisiteType) {
    ...
    // the app's only revoke that happens inside Easymatic, and both callers re-read grant
 ...
 * out of Easymatic mid-tap). So Revoke means "take you to where it is done".
 */
...
/**
 * The dialog that turns Easymatic into a device administrator.
 *
...
private const val NOTIFICATION_LISTENER_CLASS =
    "io.github.m1n1m1.easymatic.data.trigger.NotificationListener"

private const val DEVICE_ADMIN_RECEIVER_CLASS =
    "io.github.m1n1m1.easymatic.data.trigger.LoginAdminReceiver"
