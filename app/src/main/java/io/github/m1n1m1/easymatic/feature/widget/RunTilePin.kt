...
object RunTilePin {
    ...
    internal const val EXTRA_TRIGGER_KEY = "io.github.m1n1m1.easymatic.RUN_TILE_TRIGGER"
}
...
class RunTilePinnedReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (appWidgetId == AppWidgetManager.INVALID_APPWIDGET_ID || triggerKey.isNullOrBlank()) {
            Log.w("Easymatic", "Run tile pinned with no widget id or trigger; nothing to configure")
            return
        }
...
private suspend fun adoptRunTileTrigger(context: Context, appWidgetId: Int, triggerKey: String) {
    ...
    try {
        ...
        Log.w("Easymatic", "Could not configure pinned Run tile $appWidgetId", e)
    }
}
