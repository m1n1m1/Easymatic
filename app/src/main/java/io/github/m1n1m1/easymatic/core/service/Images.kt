 ...
 * through `io.github.m1n1m1.easymatic.domain.model.ImageRef`, and accepting both is forced rather
...
interface Images {
     ...
     *
     * **The result is always a row Easymatic owns**, so no consent ladder applies on any
     ...
     *
     * **Headless.** Easymatic opens the camera itself through camera2 — no camera app, no
 ...
 * express: the photo *has* a location and Easymatic is not allowed to read it, which from
...
data class ImageEdit(
    ...
    /** Absolute folder for the result. Blank means the app's own `Pictures/Easymatic`. */
    val toFolder: String = "",
