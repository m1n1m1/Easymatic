...
import io.github.m1n1m1.easymatic.nodeapi.plugin.RejectedPluginNode
...
data class InstalledPlugin(
     ...
     *
     * Distinct from [problem], which is Easymatic's own sentence about a plugin it could
    ...
    private suspend fun fetch(channel: BinderPluginChannel, packageName: String): Result<PluginManifestWire> {
        ...
        return runCatching { PluginJson.decodeFromString(PluginManifestWire.serializer(), json) }
            .recoverCatching { cause ->
                ...
                throw Problem("Sent something Easymatic could not read.")
            }
    }
