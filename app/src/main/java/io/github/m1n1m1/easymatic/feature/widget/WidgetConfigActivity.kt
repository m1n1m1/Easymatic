 ...
 * than the widget's Material You — `EasymaticTheme(darkTheme = true,
    ...
    protected fun confirm(apply: suspend (androidx.glance.GlanceId) -> Unit) {
        lifecycleScope.launch {
            try {
                ...
                Log.w("Easymatic", "Could not configure widget $appWidgetId", e)
            }
            finish()
        }
    }
...
class PanelConfigActivity : WidgetConfigActivity() {
    ...
    override fun Content() {
        ...
        ConfigScaffold(
            title = stringResource(R.string.widget_easymatic_panel),
            action = {
                Button(
                    onClick = {
