...
offered to you and to Easymatic's own nodes alike — the one chooser that is not the host's
to withhold, because what it reaches is another app on the phone rather than anything of
the user's that Easymatic keeps.

### `@PluginChoice` — a chooser over **your** lists

What you almost certainly wanted `@Picker` for. Easymatic's rule is that an identifier a
...
`@PluginChoice` moves the authority rather than the boundary. Easymatic asks **you**:
...
- **`config` carries the fields you named in `scopedBy`**, and nothing else — everything
  ...
  field lists that space's boards) and it is deliberately not the whole form: Easymatic
...
- **Scoping is unchanged.** `scopedBy` still narrows, still arrives in `request.config`, and
  Easymatic still clears the field when the scope changes.
...
touches nothing of Easymatic's, and everything it can obtain is something your own Activity
...
and no grant handling. Easymatic builds the launch from what you declared, reads the answer
...
- **Reach for `OPEN_DOCUMENT`, not `GET_CONTENT`.** `GET_CONTENT`'s grant dies with the
  ...
  is persistable and Easymatic takes it.
...
- **A `content://` value is readable in your process, for the length of the call and no
  longer.** Easymatic lends the grant by package immediately before your `execute` and takes
...
- **Taking a photograph** needs a camera app (near-universal) *and* Easymatic's CAMERA grant,
  which is not automatic: Easymatic declares `CAMERA` for its torch node, and Android then
...
in is the failure Easymatic cannot see from outside: the graph is perfect and the macro
...
time leaves Easymatic saying nothing at all, which is deliberate — a panel that badges every
node on a guess is worse than one that waits until it knows.

## A settings screen

Easymatic withholds `@ApiToken` and its credential libraries from plugins, so your sign-in
lives in your own app. Export an Activity under `io.github.m1n1m1.easymatic.action.PLUGIN_SETTINGS`
...
Easymatic resolves this against **your package** through `PackageManager` and launches the
...
name, so the published id is `plugin:com.acme.tools/shout`. Easymatic derives the same
...
Declare what you need in **your own** manifest and request it yourself. Easymatic
...
Easymatic runs your manifest through before it will show a single node — over your own
...
A plugin that is merely installed contributes nothing. It appears on Easymatic's
...
also lists any node Easymatic refused, with the reason — the first place to look when
...
### Your node's text is never translated by Easymatic

Easymatic resolves its own node names, descriptions, port labels and `@Label` config
...
your `@Label` into a plain `String` inside your own process, long before Easymatic sees
it — so there is no key for Easymatic to look up and never will be. Its resource ids
...
your service before building the declaration. Easymatic re-reads your declarations when
it binds, so the locale in force at that moment is the one the user sees.

## Lifetimes

Easymatic binds your service while a macro using your nodes is armed, while the graph
...
When it comes back, Easymatic **re-arms rather than resumes**: your triggers are armed
...
`PLUGIN_PROTOCOL_VERSION` is stamped onto your manifest by the SDK, and Easymatic refuses a
