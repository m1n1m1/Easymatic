 ...
 * commands, so it can go stale silently. These pages cannot: astro.config.mjs imports
 * this module, so the producer runs inside the only consumer there is — `dev`, `build`,
 * `check` and a bare `npx astro build` alike. Committing 180 derived files would buy a
 * guarantee that already holds and cost a 7 000-line diff on every template tweak.
 *
 ...
 *
 * The sidebar is built alongside them and returned as this module's default export, and that
...
// Resolved from this file rather than from process.cwd(). astro.config.mjs imports this
// module for its side effects, and a config is loaded with the cwd of whoever invoked
// Astro — which is website/ for `npm run build` and need not be for anything else.
// Anchoring to the script makes the generator correct from any working directory.
const WEBSITE = join(dirname(fileURLToPath(import.meta.url)), '..');

const EXPORT = join(WEBSITE, '..', 'docs', 'nodes.generated.json');
const PROSE_DIR = join(WEBSITE, '..', 'docs', 'nodes');
const OUT_DIR = join(WEBSITE, 'src', 'content', 'docs', 'docs', 'reference', 'nodes');
...
function buildSidebar(groups) {
  // Annotated because astro.config.mjs is `// @ts-check`ed and now imports this directly:
  // without it the four kinds are properties of an inferred `{}` and every lookup there is
  // a ts(2339). `any` for the items, whose shape is Starlight's rather than ours.
...
// Returned rather than written to src/generated/node-sidebar.mjs, which is what this used
// to do. astro.config.mjs is the only consumer, and it imports this module for its side
// effects anyway — so a file in between meant the config had to import a module that did
// not exist yet when Vite resolved the config's imports, which is a crash on the first
// build of a clean checkout no matter how early the generator runs.
