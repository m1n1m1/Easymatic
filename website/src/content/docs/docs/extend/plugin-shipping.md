...
Easymatic derives the same prefix from what the system reports for your service and
...
Declare what you need in **your own** manifest and request it yourself. Easymatic checks
...
That screen also lists **any node Easymatic refused, with the reason** — the first place
...
your `@Label` into a plain `String` inside your own process, long before Easymatic sees it
— so there is no key for Easymatic to look up and never will be. Its resource ids would be
...
your service before building the declaration. Easymatic re-reads your declarations when it
...
`PLUGIN_PROTOCOL_VERSION` is stamped onto your manifest by the SDK, and Easymatic refuses
