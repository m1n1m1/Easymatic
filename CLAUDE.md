...
`PrerequisiteType` grew `BATTERY_OPTIMISATION`, `EXACT_ALARM` and `WRITE_SETTINGS` rather than the screen keeping a parallel list, because all three are the shape the enum already models — one system-wide switch, its own API, no runtime dialog — and the two exhaustive `when`s over it (`AndroidPermissionChecker.isPrerequisiteSatisfied`, `openSettingsFor`) are what make a fourth impossible to add silently. A test pins that every grantable type has a row.
