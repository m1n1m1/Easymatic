...
**Backup is one include list, and everything reads it.** `BackupContents.included` (`domain/backup/`) names the ten `filesDir` entries that are "settings and macros". The Backup screen's ZIP (`data/backup/BackupRepository`) carries exactly those, and `res/xml/backup_rules.xml` / `data_extraction_rules.xml` `<include>` exactly those, so Android's Auto Backup and device transfer carry the same set. It is an *include* list on purpose — an exclude list would sweep in Glance's DataStore, WorkManager's database and the restore staging area — and `BackupContentsTest` pins both XML files to it and pins it to the directories the repositories actually create, so a new library fails the build until it is classified. Restore replaces the libraries and *adds* the workflows (a colliding id lands as a disarmed copy, `placeWorkflow`), then every `ReloadableLibrary` re-reads its file, because each JSON library caches at construction. The re-arm is the ViewModel's line (`ACTION_REARM_CHANGED`), since `data` may not reach `engine`. Credentials travel sealed, plus the **escrow** (`SecretEscrow`, `secrets/escrow.json`, in the include list): every credential again under a key derived from a **backup password** (`PasswordCipher`, PBKDF2 + AES-GCM), kept in step with the three credential libraries through `EscrowedLibrary` and three collected flows. **A backup always has a password, and the whole file is under it**: the file is a plaintext manifest line (which carries the salt and a verifier, nothing that opens anything) followed by the zip encrypted by `ArchiveCipher` — AES-CTR plus HMAC-SHA256, streaming, because GCM would hold the whole archive in memory — under the second half of the same PBKDF2 derivation the escrow key comes from. Backing up asks for the password *before* the picker and re-keys the escrow so the file's escrow matches; restoring asks in the confirmation, checks the header's verifier (`EscrowChallenge`) *before anything is touched*, decrypts with the cipher that check derived and hands the same cipher to `unlockWith`, so the password is derived once. `MIN_PASSWORD_LENGTH` is enforced in the dialog and in `setPassword`, because the file's salt and verifier make guessing offline. An escrow with entries but no verifier reads as none (`isConsistent`). The derived key is itself Keystore-sealed in the file, so the phone that set the password never asks again, while an Android restore lands `Locked` and `MainActivity.onResume` asks once. Unlocking never overwrites a credential that already opens.
