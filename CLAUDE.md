...
`AiConnectionRepository` (`data/`) is an ordinary library, on `MailAccountRepository`'s shape: `AiConnection` is an id, a name, an `AiProvider` and a sealed key, and `action.ai_prompt` names one through `@Picker(PickerKind.AI_CONNECTION)`. **It started as a single key and the reversal is the instructive part.** The first cut argued in its own KDoc that one key per phone was the whole model — nobody has two Gemini keys, so a picker would invent a decision for symmetry. Two things overturned it: a second provider is a matter of when rather than whether, and it is the *library shape* rather than the provider enum that is expensive to retrofit; and **quota is per key**, so separating a macro that fires every five minutes from one that summarises a mail is a real use for two keys with one provider. `AiProvider` shipped with one member and existed anyway, for `SmartHomeKind`'s reason — and **the bet paid**: adding four providers touched no workflow, no node, no schema version and nothing in `core/` or `engine/`.
...
**Blank is not an answer, and no connection is implicitly default.** An implicit default is invisible on the card, so the day a second connection is added every existing node would silently keep billing whichever sorted first — which, quota being per key, is a real consequence and not a cosmetic one. `AiConnections` (`domain/registry/`) is the **fourth hydrated registry**, after `MacroDirectory`, `GlobalVariables` and `SmartHomeHubs`, so `validateAiConnectionRefs` can warn about the unset case, the deleted case and — since the open-ended providers arrived — the **unfinished** one. It publishes two sets rather than one filtered list, because `exists` must go on seeing a half-finished connection or the panel would call it deleted, which is the wrong sentence about a connection sitting right there in the list. Three states, three fixes, three sentences. The unfinished one earns its place most: a self-hosted connection with no server address renders *perfectly* — name, provider, key — and answers nothing.
...
**A connection carries its own system prompt, combined with the node's rather than replacing it**, connection first, joined by a blank line (`combineInstructions`, in `data/` because that is where the connection can be resolved — `core/` and `engine/` never learn the field exists). The two answer different questions: the connection's is the persona and house rules that should hold wherever it is used, the node's is the one task. An override would mean any node setting a single task instruction silently threw the connection's rules away, with nothing on the card saying so. Either half alone passes through untouched, which is what makes the field invisible to every macro written before it.

**`AiModel` names a trade-off, not a product**, and that is load-bearing rather than tidy. A workflow persists the enum's *name*, and an unknown name is a discarded schema rather than a migration — so a config holding `GEMINI_2_0_FLASH` would break every saved macro the day that id was retired, with the user having chosen nothing wrong. Published ids churn on a scale of months. `FAST`/`BALANCED`/`THOROUGH` outlive them, and the mapping lives in each protocol's `modelId` where a rename is one line nothing persisted notices. Nothing vendor-shaped crosses `core`: the endpoint, the envelope, the key header and the model ids all stop at `data/ai/`, so an on-device Gemini Nano implementation would be a new `AiProtocol` and no edit here, in `engine/`, or in the node.

The **per-connection override** (`modelIdFor`) is the second half of that answer and was added when the tables' churn stopped being hypothetical: an override turns "wait for an app update" into a text field. `OPENROUTER` and `OPENAI_COMPATIBLE` publish no table at all — a catalogue of hundreds and a machine serving exactly one, neither of which has three obvious tiers — so there the ids are required, and a tier left blank falls back to the **Fast** one, which is what somebody means when their server serves a single model.
...
`action.ai_agent` ("Ask AI (with tools)") is the node that turns the app into a **harness**: it hands a model a set of Ottomatic's own nodes and macros and lets it read and act until it has an answer. `action.ai_describe` is the other node built on the same machinery.

**`action.ai_extract` and `action.ai_classify` were built here and removed on 2026-08-15**, and the reason is worth keeping because it is about *scope* rather than about a fault. Both worked, and both were built the interesting way — one tool whose *arguments* are the declared fields, one tool whose single argument is an `enum` of the branches, so the provider itself enforces the shape and `AiProtocol` gained nothing for either. What they were not is *needed*: `action.ai_agent` already answers structured questions through the same loop, and `transform.json_read` was already the road for a reply that carries shape. A second way to do a thing the graph could already do is a second thing to keep working. If either is ever wanted back, the mechanism to reach for is that one — a single tool whose arguments are the answer — and not any provider's structured-output facility.
...
**Every tool call is written to the run log as it happens**, at INFO with its arguments, and that is the only record anywhere of what an unattended macro let a model do. There is deliberately **no confirmation prompt**: the point of this app is a macro that fires unattended, and a dialog nobody is there to answer would make the feature useless rather than safe. What makes it safe is that the author chose the tools and pinned their identifiers.
