...
import com.example.ottomatic.plugin.PluginChoiceSource
...
import com.example.ottomatic.plugin.routes
 ...
 *
 * The `source` strings are this plugin's own keys and mean nothing to Ottomatic; it passes
...
data class PostConfig(
    ...
    /**
     * The third chooser, and the one Ottomatic does not draw.
     *
     ...
     * field is `ChoiceChooser.SCREEN`: the plugin exports an Activity, Ottomatic opens it
...
class PostAction : PluginAction<PostConfig, Posted>, PluginChoiceSource<PostConfig> {
     ...
     * *not* here: no Ottomatic facade, no host library, no way to read a variable or a
...
/** This plugin's key for "which space?". Opaque to Ottomatic, which echoes it back. */
