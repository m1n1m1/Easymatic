 ...
 * API but the point of it: Easymatic withholds `@ApiToken` and its credential libraries
 * from plugins because those are the host's trust boundary, and a plugin that kept its
 * user's credentials inside Easymatic would be asking the host to hold something it
 * cannot verify, refresh or revoke.
 *
 * What Easymatic *does* provide, and did not until protocol 2, is a way to reach the
 * screen where the sign-in happens — see [SampleSettingsActivity] — and a way for the
 * plugin to say it has not happened yet, which is `BaseEasymaticPluginService.status()`.
 */
