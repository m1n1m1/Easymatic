 ...
 * reason `NodeDeclarationRules` has three: [BaseOttomaticPluginService] runs it when it
 ...
 * Ottomatic as a dead transaction, which it can only report as *"Could not be reached"* —
...
object PluginNodeContracts {
     ...
     *
     * The same `when` [BaseOttomaticPluginService] uses to find a typeId, kept here rather
