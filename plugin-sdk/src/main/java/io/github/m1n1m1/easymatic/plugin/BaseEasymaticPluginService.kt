 ...
 *     <intent-filter><action android:name="io.github.m1n1m1.easymatic.action.PLUGIN" /></intent-filter>
 * </service>
 * ```
 *
...
abstract class BaseEasymaticPluginService : Service() {
     ...
     * for, so Easymatic's Problems panel is silent and correct while every node of a
     * signed-out plugin does nothing: "configured perfectly, does nothing", with nothing
     * anywhere saying why.
     *
     * Answer `PluginStatusWire(ready = false, message = "…")` and Easymatic raises that
     ...
     * network. A plugin that does not answer in time leaves Easymatic saying nothing,
    ...
    private val manifestJson: String by lazy {
        ...
        // Easymatic as a dead transaction it can only report as "could not be reached".
    ...
    private val binder = object : IEasymaticPlugin.Stub() {

        override fun declarations(): String = manifestJson

        override fun status(): String = runCatching {
            PluginJson.encodeToString(PluginStatusWire.serializer(), this@BaseEasymaticPluginService.status())
        }.getOrElse { cause ->
        ...
        override fun runAction(typeId: String, requestJson: String): String {
            ...
            return runCatching {
                PluginJson.encodeToString(
                    io.github.m1n1m1.easymatic.nodeapi.wire.ActionResultWire.serializer(),
    ...
    private fun failedAction(reason: String): String = PluginJson.encodeToString(
        io.github.m1n1m1.easymatic.nodeapi.wire.ActionResultWire.serializer(),
        io.github.m1n1m1.easymatic.nodeapi.wire.ActionResultWire(
